...
/**
 * Soft-Logic Duplikat-Erkennung
 * Vergleicht URLs basierend auf Stamm-URL und Service-Pattern
 */
function isDuplicate(newUrl, existingUrl) {
  ...
  // Stamm-URL Vergleich (ohne spezifische Pfade)
...
async function addNewWFSStreams() {
  console.log('🔄 Starte Import der INSPIRE-WFS-URLs 2025...');
  ...
  for (const newStream of INSPIRE_WFS_2025) {
    ...
    for (const existingStream of existingStreams) {
      if (isDuplicate(newStream.url, existingStream.url)) {
        duplicates.push({
          new: newStream,
          existing: existingStream,
          reason: 'URL-Duplikat erkannt'
        });
  ...
  if (duplicates.length > 0) {
    ...
    duplicates.forEach((dup, index) => {
      ...
      console.log(`   Vorhanden: ${dup.existing.url}`);
